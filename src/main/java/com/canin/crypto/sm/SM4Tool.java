...
public class SM4Tool {
    ...
    private byte[] key;//length must be 16
        ...
        if (workMode != null) {
            switch (workMode) {
                ...
                case CBC:
                    ...
                    break;
                case CTR:
                    blockCipher = new SICBlockCipher(new SM4Engine());
                    break;
                case CFB:
                    blockCipher = new CFBBlockCipher(new SM4Engine(), refluxBits);
                    break;
                case OFB:
                    blockCipher = new OFBBlockCipher(new SM4Engine(), refluxBits);
                    break;
                default:
            }
        }
    ...
    public SM4Tool(boolean isEncryption, byte[] key, byte[] iv, WorkMode workMode, Paddings padding, int refluxBits) {
        init(isEncryption, key, iv, workMode, padding, refluxBits);
    }

    public SM4Tool(boolean isEncryption, byte[] key, byte[] iv, WorkMode workMode, Paddings padding) {
